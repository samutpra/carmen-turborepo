...
import { DepartmentLabel, DepartmentSchema, DepartmentType, PayloaDepartmentType } from '@/lib/types';
    ...
    const confirmDelete = async () => {
        try {
            ...
            if (idToDelete) {
                ...
                setDepartments(prev => prev.filter(item => item.id !== idToDelete));
                fetchData();
    ...
    const handleSave = async (data: DepartmentType) => {
        try {
            ...
            if (editingItem?.id) {
                ...
                const updatedCurrency = await updateDepartment(token, editingItem.id, updatedFields);
                setDepartments(prev => prev.map(item => (item.id === editingItem.id ? updatedCurrency : item)));
            } else {
