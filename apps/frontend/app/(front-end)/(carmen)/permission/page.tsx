...
const PermissionPage = () => {
    ...
    const supportBossBU1: UserType = { id: '2', role: 'support', subRole: 'boss', bu: 'BU1' };
    const supportSubordinateBU1: UserType = { id: '3', role: 'support', subRole: 'subordinate', bu: 'BU1' };
    const userBossBU1: UserType = { id: '4', role: 'user', subRole: 'boss', bu: 'BU1' };
    const userSubordinateBU1: UserType = { id: '5', role: 'user', subRole: 'subordinate', bu: 'BU1' };

    const supportBossBU2: UserType = { id: '6', role: 'support', subRole: 'boss', bu: 'BU2' };
    const userSubordinateBU2: UserType = { id: '9', role: 'user', subRole: 'subordinate', bu: 'BU2' };
    const userBossBU2: UserType = { id: '8', role: 'user', subRole: 'boss', bu: 'BU2' };

    const [user, setUser] = useState<UserType>(adminUser);
    const [comments, setComments] = useState<ResourceType[]>(mockComments);
    const [editingComment, setEditingComment] = useState<ResourceType | null>(null);
    ...
    const handleCreate = () => {
        ...
        const newComment: ResourceType = {
            ...
            ownerId: user.id,
            bu: user.bu || 'BU1', // ใช้ BU ของผู้ใช้ หรือค่าเริ่มต้นสำหรับ admin
            visibility: 'public',
            content: 'New Comment'
        };
    ...
    const handleUpdate = (commentId: string, newContent: string) => {
        ...
        if (commentToUpdate && hasPermission(user, 'update:comments', commentToUpdate)) {
    ...
    const handleDelete = (commentId: string) => {
        ...
        const canDelete = hasPermission(user, 'delete:comments', commentToDelete) ||
        ...
        } else if (hasPermission(user, 'delete:subordinateComments', commentToDelete)) {
            deleteType = 'Boss Delete';
        } else if (hasPermission(user, 'delete:ownComments', commentToDelete)) {
            deleteType = 'Own Delete';
        }

        if (confirm(`Are you sure you want to delete this comment? (${deleteType})`)) {
    ...
    const getUserRoleDisplay = (user: UserType) => {
        const roleParts = [
            ...
            user.subRole && user.subRole.charAt(0).toUpperCase() + user.subRole.slice(1),
            user.bu && `(${user.bu})`
        ].filter(Boolean);
            ...
            <div className="space-y-6">
                ...
                <div className="bg-slate-100 p-4 rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="px-2 py-1 bg-blue-100 rounded">{getUserRoleDisplay(user)}</span>
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <Button onClick={() => setUser(supportBossBU1)}>Support Boss (BU1)</Button>
                    <Button onClick={() => setUser(supportSubordinateBU1)}>Support Staff (BU1)</Button>
                    <Button onClick={() => setUser(userBossBU1)}>User Boss (BU1)</Button>
                    <Button onClick={() => setUser(userSubordinateBU1)}>User Staff (BU1)</Button>
                    
                </div>

                <div className="flex flex-wrap gap-2">
                    <Button onClick={() => setUser(supportBossBU2)}>Support Boss (BU2)</Button>
                    <Button onClick={() => setUser(supportBossBU2)}>Support Boss (BU2)</Button>
                    <Button onClick={() => setUser(userBossBU2)}>User Boss (BU2)</Button>
                    <Button onClick={() => setUser(userSubordinateBU2)}>User Staff (BU2)</Button>
                </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {visibleComments.map((comment) => {
                                const commentOwner = mockUsers.find(u => u.id === comment.ownerId);
                                return (
                                    <TableRow key={comment.id}>
                                        ...
                                        <TableCell>
                                            <span className="px-2 py-1 bg-purple-100 rounded">
                                                {commentOwner ? getUserRoleDisplay(commentOwner) : 'Unknown'}
                                            </span>
                                        </TableCell>
                                        <TableCell>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                    <Card className="w-96 p-4">
                        ...
                        <CardFooter className="flex justify-between">
                            ...
                            <Button
                                onClick={() => {
                                    const newContent = (document.getElementById('comment-content') as HTMLInputElement).value;
