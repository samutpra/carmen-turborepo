...
import { PrismaClient as dbTenant, product_sub_category_table } from '@prisma-carmen-client-tenant';
...
export class ProductSubCategoryService {
	...
	async _getById(db_tenant: dbTenant, id: string): Promise<product_sub_category_table> {
	...
	async findOne(req: Request, id: string): Promise<ResponseSingle<product_sub_category_table>> {
	...
	async findAll(req: Request, q: QueryParams): Promise<ResponseList<product_sub_category_table>> {
		const { user_id, business_unit_id } = this.extractReqService.getByReq(req);
		this.db_tenant = this.prismaClientMamager.getTenantDB(business_unit_id);
		...
		const listObj = await this.db_tenant.product_sub_category_table.findMany(q.findMany());
	...
	async create(req: Request, createDto: ProductSubCategoryCreateDto): Promise<ResponseId<string>> {
		const { user_id, business_unit_id } = this.extractReqService.getByReq(req);
		this.db_tenant = this.prismaClientMamager.getTenantDB(business_unit_id);
	...
	async update(req: Request, id: string, updateDto: ProductSubCategoryUpdateDto) {
		const { user_id, business_unit_id } = this.extractReqService.getByReq(req);
		this.db_tenant = this.prismaClientMamager.getTenantDB(business_unit_id);
		const oneObj = await this._getById(this.db_tenant, id);
