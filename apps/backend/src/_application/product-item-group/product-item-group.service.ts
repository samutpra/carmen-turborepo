...
import { PrismaClient as dbTenant, product_item_group_table } from '@prisma-carmen-client-tenant';
...
export class ProductItemGroupService {
	...
	async _getById(db_tenant: dbTenant, id: string): Promise<product_item_group_table> {
	...
	async findOne(req: Request, id: string): Promise<ResponseSingle<product_item_group_table>> {
	...
	async findAll(req: Request, q: QueryParams): Promise<ResponseList<product_item_group_table>> {
		const { user_id, business_unit_id } = this.extractReqService.getByReq(req);
		this.db_tenant = this.prismaClientMamager.getTenantDB(business_unit_id);
		const max = await this.db_tenant.product_item_group_table.count({
			where: q.where()
		});
		const listObj = await this.db_tenant.product_item_group_table.findMany(q.findMany());
	...
	async create(req: Request, createDto: ProductItemGroupCreateDto): Promise<ResponseId<string>> {
		const { user_id, business_unit_id } = this.extractReqService.getByReq(req);
		this.db_tenant = this.prismaClientMamager.getTenantDB(business_unit_id);
