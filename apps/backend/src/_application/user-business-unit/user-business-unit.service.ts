...
import { PrismaClient as dbSystem, user_business_unit_table } from '@prisma-carmen-client-system';
...
export class UserBusinessUnitService {
	...
	async _getById(db_System: dbSystem, id: string, user_id: string): Promise<user_business_unit_table> {
	...
	async findOne(req: Request, id: string): Promise<ResponseSingle<user_business_unit_table>> {
	...
	async findAll(req: Request, q: QueryParams): Promise<ResponseList<user_business_unit_table>> {
		const { user_id, business_unit_id } = this.extractReqService.getByReq(req);
		this.db_System = this.prismaClientMamager.getSystemDB();
		const q_where = {
			...q.where(),
