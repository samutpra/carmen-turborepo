...
export class UserPermissionService {
	...
	async getPermissions(req: Request, permission: string, asList: boolean) {
		...
		const roles = await this.db_System.user_application_role_table.findMany({
			where: {
				user_id: user_id
			}
		});

		const permissions = await this.db_System.application_role_permission_table.findMany({
			where: {
				application_role_id: {
